...
for (let dataCollectorButton of dataCollectorButtons) {
...
async function loadMobileNetFeatureModel() {
    ...
    mobilenet = await tf.loadLayersModel(URL, {fromTFHub: true})
    ...
    // for (let layer of mobileNetBase.layers) {
...
async function enableCam() {
    if (hasGetUserMedia()) {
        ...
        console.warn("Not supported by your browser")
    }
}
...
async function trainAndPredict() {
    ...
    let results = await model.fit(inputsAsTensor, oneHotOutputs, {
        ...
        batchSize: 5,
        epochs: 5,
        callbacks: {onEpochEnd: (epoch, data) => {
...
function predictLoop() {
    if (predict) {
        tf.tidy(function() {
            ...
            STATUS.innerText = "Prediction: " + CLASS_NAMES[highestIndex] + " with " + Math.round(predictionArray[highestIndex] * 100) + "% confidence"
        });
